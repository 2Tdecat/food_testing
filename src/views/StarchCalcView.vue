...
import StarchPanel from '@/components/sugar/StarchPanel.vue'
import { addStarchRecord } from '@/utils/history'
...
const panelRef = ref<InstanceType<typeof StarchPanel>>()
...
<template>
  <div class="page">
    <t-navbar title="淀粉（1/2法、正/反滴）" fixed placeholder @left-click="router.back()">
      <template #left>
        <t-icon name="chevron-left" size="24px" />
      </template>
      <template #right>
        <t-icon name="history" size="22px" @click="router.push('/tools/history/starch')" />
      </template>
    </t-navbar>
    ...
    <div class="body">
      <t-notice-bar
        class="notice"
        content="淀粉滴定测定：正滴（直接滴定）/反滴（反滴定）切换，淀粉1法/2法切换，计算公式与实验室《糖》表四个淀粉表一致，平行测定结果取算术平均值"
        marquee
      />
      ...
      <div class="card sample-card">
        ...
        <t-cell-group bordered>
          <t-input v-model="sampleName" label="样品名称" placeholder="如 红薯淀粉" align="right" />
          <t-input v-model="sampleNo" label="样品编号" placeholder="如 35393-1" align="right" />
        </t-cell-group>
      </div>

      <!-- 淀粉滴定测定 -->
      <StarchPanel ref="panelRef" />
